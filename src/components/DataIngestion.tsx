  ...
  const handleFileUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (points.length === 0) {
          setError('No valid rainfall rows were detected in the selected file.');
        } else {
          addRainData(points);
        }
      } catch (parseError) {
        console.error('Parse Error', parseError);
        setError('Unable to parse this file. Please verify format and try again.');
      } finally {
        setIsParsing(false);
        e.target.value = '';
      }
      ...
      <CardContent className="space-y-4">
        <div className="relative">
          ...
          >
            ...
            <span className="text-sm font-medium text-slate-300">{isParsing ? 'Parsing...' : 'Drop CSV or Click'}</span>
            <span className="text-xs text-slate-500 mt-1">Format: Timestamp, Value (in)</span>
          </div>
        </div>

        {error && (
          <div className="bg-red-950/40 border border-red-900/60 rounded-md px-3 py-2 text-xs text-red-300 flex items-center gap-2">
